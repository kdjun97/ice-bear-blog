...
내용을 수정하거나 추가하고, pull request로 원작자에게 바뀐 내용을 merge를 할건지 묻는다.  
만약 원작자가 pull request를 받아들이면, 원작자의 원본 레포지토리가 바뀌게 된다.  
pull request 없이는, 나의 깃허브에 forked된 레포지토리에서만 작업을 할 수 있다.  

아직 Github계정이 없다면 당장 [Github 사이트](https://github.com/) 에서 회원가입 하길 바란다.  
